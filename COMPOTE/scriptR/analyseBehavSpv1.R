 ...
 meanPtot<-with(mt,by(pct_infected_tot,list(radiusInfestMax,tpsExtermination,SpatialModality),mean))
 meanPtot<-array(as.vector(meanPtot),dim=c(6,6,4))
 minPtot<-with(mt,by(pct_infected_tot,list(radiusInfestMax,tpsExtermination,SpatialModality),min))
 minPtot<-array(as.vector(minPtot),dim=c(6,6,4))
 maxPtot<-with(mt,by(pct_infected_tot,list(radiusInfestMax,tpsExtermination,SpatialModality),max))
 ...
 plot(1:32,type="n",xlab="Radius",ylab="Infestation",bty="l",ylim=c(0,1))
 for(i in 1:6){
   for(j in  1:4){
 ...
 windows()
 plot(1:32,type="n",xlab="Radius",ylab="Infestation",bty="l",ylim=c(0,1), main="for slow growth desease (t=120)")
 for(i in 4:4){
   for(j in  1:4){
 ...
 windows()
 boxplot(pct_var1~radiusInfestMax+tpsExtermination+SpatialModality,data=mt)
 meanPtot<-with(mt,by(pct_infected_tot,list(radiusInfestMax,tpsExtermination,SpatialModality),mean))
 meanPtot<-array(as.vector(meanPtot),dim=c(6,6,4))
 minPtot<-with(mt,by(pct_infected_tot,list(radiusInfestMax,tpsExtermination,SpatialModality),min))
 minPtot<-array(as.vector(minPtot),dim=c(6,6,4))
